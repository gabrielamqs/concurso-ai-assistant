...
import { LogOut, Home, CreditCard, Bell, HelpCircle } from "lucide-react";
...
type ViewType = "landing" | "auth" | "dashboard" | "pricing";
...
export default function App() {
  ...
  useEffect(() => {
    if (currentView === "dashboard" && user) {
      const hasSeenTour = localStorage.getItem("concursoai_tour_completed");
      if (!hasSeenTour) {
  ...
  useEffect(() => {
    const handleOpenEditalChat = (event: CustomEvent) => {
      const edital = mockEditais.find((e) => e.id === event.detail.editalId);
      if (edital) {
        setSelectedEdital(edital);
      }
    };

    window.addEventListener("openEditalChat", handleOpenEditalChat as EventListener);
    return () => {
      window.removeEventListener("openEditalChat", handleOpenEditalChat as EventListener);
    };
  }, []);
  ...
  if (!user && currentView !== "landing" && currentView !== "pricing") {
  ...
  return (
    <div className="flex h-screen bg-white">
      <div className="flex-1 flex flex-col min-w-0">
        <header className="border-b border-gray-200 bg-white px-4 py-3 flex items-center gap-3">
          <div className="flex items-center gap-3">
            <img src={logoImage} alt="ConcursoAI" className="w-8 h-8" />
            <div>
      ...
      <WelcomeTour isOpen={showWelcomeTour} onClose={handleCloseTour} />
      <Toaster />
    </div>
  );
}